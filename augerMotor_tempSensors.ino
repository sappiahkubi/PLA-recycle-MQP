...
void setup() {
  ...
  digitalWrite(8,HIGH); //Set direction to be the opposite way
...
void loop() {
  ...
  for(x=0;x<680;x++) //this loop dictates the distance it travels (or maybe the time that it travels??)

  {
    digitalWrite(9,LOW); //pulse (move) the motor 
    delayMicroseconds(500); //wait (pause) for 500 microseconds 

    digitalWrite(9,HIGH); //stop the motor 
    delayMicroseconds(500); //wait (pause) for 500 microseconds 
